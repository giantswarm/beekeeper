...
func TestARenamedSupervisorIsKnownByItsCurrentTitle(t *testing.T) {
	st := handOverState()
	renamed := []*claude.Session{{PID: 100, ID: supA.Session, HostID: supA.HostSession, Name: "Supervisor run 18"}}
	...
	if !changed || st.Supervisor.Name != "Supervisor run 18" || st.SupervisorCLI.Supervisor.Name != "Supervisor run 18" {
		t.Fatalf("the rename is not recorded: %v %+v %+v", changed, st.Supervisor, st.SupervisorCLI)
	}
	if len(evs) != 1 || evs[0].Verb != "supervisor.rename" || !strings.Contains(evs[0].Detail, `is now "Supervisor run 18"`) {
	...
	if renameHolder(st.Supervisor, []*claude.Session{{PID: 100, ID: supA.Session}}) || st.Supervisor.Name != "Supervisor run 18" {
