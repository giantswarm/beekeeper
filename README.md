...
| `beekeeper guide queue` | The guide's queue: every open note filed for its person, `guide.person` (`note add --for`, or an older note's `[for <person>]` text prefix; any case), with its owning session (the one that filed it, and whether it still runs), deadline and default, then every session the desktop files as waiting on its person (its session record's latest turn summary is `blocked` with an action; no transcript is read). With `guide.person` unset, every note filed `--for` anyone, and a line that says so. Delta output like `sessions`. |
